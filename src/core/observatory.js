...
const BRIGHT_STARS = [
  ...
  { name: 'Betelgeuse', ra: 5.919, dec: 7.407, mag: 0.50, color: 0xff6644 },
  ...
  { name: 'Polare', ra: 2.530, dec: 89.264, mag: 1.97, color: 0xffeecc },
];
...
export function createStarDome(radius) {
  ...
  const starMaterial = new THREE.ShaderMaterial({
    ...
    vertexShader: 'attribute float size; attribute vec3 color; varying vec3 vColor; uniform float time; uniform float twinkleAmount; void main() { vColor = color; vec4 mvPosition = modelViewMatrix * vec4(position, 1.0); gl_PointSize = size * (300.0 / -mvPosition.z); float twinkle = 1.0 + twinkleAmount * sin(time * 2.0 + position.x * 10.0) * cos(time * 3.0 + position.y * 8.0); gl_PointSize *= twinkle; gl_Position = projectionMatrix * mvPosition; }',
    fragmentShader: 'varying vec3 vColor; void main() { float dist = length(gl_PointCoord - vec2(0.5)); if (dist > 0.5) discard; float alpha = 1.0 - smoothstep(0.0, 0.5, dist); float glow = exp(-dist * 8.0); gl_FragColor = vec4(vColor * (1.0 + glow * 2.0), alpha); }',
    transparent: true,
    depthWrite: false,
...
export function createGround(radius, color) {
  ...
  const material = new THREE.MeshStandardMaterial({ color: color, roughness: 0.95, metalness: 0.0 });
...
export class Observatory {
  ...
  toggle(planetBody, sunPos) {
    if (this.active) { this.exit(); } else if (planetBody) { this.enter(planetBody, sunPos); }
  }
}