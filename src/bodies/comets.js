...
export function buildCometOrbit(def) {
  ...
  const line = new THREE.Line(
    ...
    new THREE.LineDashedMaterial({
      ...
      dashSize: 6, gapSize: 10, transparent: true, opacity: 0.35,
    })
  );
...
export function createComets(COMETS, cGroup, hitboxList, ui, allBodies, meshList, selectBody) {
  return COMETS.map(def => {
    ...
    const cr = Math.round(cHex.r * 255), cg = Math.round(cHex.g * 255), cb = Math.round(cHex.b * 255);
    const glow = makeCanvasSprite((ctx, s) => {
    ...
    const tail = new THREE.Points(
      tailGeo,
      new THREE.PointsMaterial({ size: 1.8, sizeAttenuation: true, vertexColors: true, transparent: true, opacity: 0.75 })
    );
    cGroup.add(tail);
    ...
    const body = {
      ...def, type: 'comet', pivot, mesh, glow, labelEl,
      visualR: 4, moons: 0, day: null, distAU: def.perielio,
    };
    allBodies.push(body);
