...
export class SpaceNews {
  ...
  _build() {
    ...
    <div id="snBody" style="flex:1;overflow-y:auto;padding:14px 18px;">
      <div style="text-align:center;padding:30px 0;opacity:0.5;">${escapeHtml(
        t('sn_loading')
      )}</div>
    </div>`;
  ...
  async _load() {
    ...
    try {
      ...
      const safeImageUrl =
        apod?.media_type === 'image' && isValidUrl(apod.url) ? escapeHtml(apod.url) : '';
      ...
      if (apod) {
        html += `<div style="margin-bottom:18px;">
          ...
          ${
            safeImageUrl
              ? `<img src="${safeImageUrl}" alt="${safeTitle}" style="width:100%;border-radius:10px;margin-bottom:8px;" loading="lazy" referrerpolicy="no-referrer" />`
              : ''
          }
        ...
        html += `<div>
          ...
          ${neos
            ...
            <div style="display:flex;gap:10px;opacity:0.7;font-size:10.5px;">
              <span>${escapeHtml(n.diameter)}</span><span>${escapeHtml(
                n.velocity
              )}</span><span>${escapeHtml(n.miss)}</span>
            </div>
          </div>`;
            })
            .join('')}
      ...
      if (!html)
        html =
          '<div style="text-align:center;padding:30px 0;opacity:0.5;">Impossibile caricare i dati NASA</div>';
      body.innerHTML = html;
    } catch {
      body.innerHTML =
        '<div style="text-align:center;padding:30px 0;opacity:0.5;">Errore di caricamento</div>';
    }
  }
}
