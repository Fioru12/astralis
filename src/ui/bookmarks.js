...
export class CameraBookmarks {
  ...
  save(CAM, name) {
    const entry = {
      name: name || `Posizione ${this.bookmarks.length + 1}`,
  ...
  _show(CAM) {
    ...
    Object.assign(this.panel.style, {
      ...
      background: 'rgba(8, 10, 20, 0.92)', backdropFilter: 'blur(16px)',
      border: '1px solid rgba(91,196,207,0.25)', borderRadius: '12px',
      color: '#e4eaf8', zIndex: '9900',
    ...
    this.panel.innerHTML = `
      ...
      <div id="bmkSaveRow" style="padding:10px 12px;display:flex;gap:6px;border-bottom:1px solid rgba(255,255,255,0.05);">
        <input id="bmkNameInput" type="text" placeholder="Bookmark name..." style="flex:1;padding:6px 8px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:6px;color:#e4eaf8;font-size:12px;">
    ...
    this.panel.querySelector('#bmkSaveBtn').onclick = () => {
      ...
      const name = input.value.trim() || `Posizione ${this.bookmarks.length + 1}`;
      this.save(CAM, name);
      input.value = '';
      this._render();
    };
  ...
  _render() {
    ...
    if (this.bookmarks.length === 0) {
      list.innerHTML = '<div style="padding:20px;text-align:center;font-size:12px;color:rgba(228,234,248,0.4);">Nessun bookmark salvato</div>';
      return;
    }
    list.innerHTML = this.bookmarks.map((bm, i) => `
