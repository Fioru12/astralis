...
export class TimeTravel {
  ...
  hide() {
    ...
    setTimeout(() => { this.panel?.remove(); this.panel = null; }, 250);
    this.isOpen = false;
    this._stopAnim();
  }
  ...
  _build() {
    ...
    this.panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
        <h2 style="margin:0;font-size:1.05rem;font-weight:800;color:#5bc4cf;">${t('timetravel_title')}</h2>
      ...
      <div style="margin-bottom:14px;">
        <label style="display:block;font-size:11px;color:rgba(228,234,248,0.5);
          text-transform:uppercase;letter-spacing:0.8px;margin-bottom:6px;">${t('timetravel_pick')}</label>
      ...
      <div style="display:flex;gap:6px;margin-bottom:16px;">
        <button id="ttJump" style="flex:1;background:linear-gradient(135deg,#5bc4cf,#8be0ea);border:none;
          color:#000;padding:9px;border-radius:8px;cursor:pointer;font-size:12.5px;font-weight:700;">${t('timetravel_jump')}</button>
        <button id="ttAnim" style="flex:1;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.16);
          color:#e4eaf8;padding:9px;border-radius:8px;cursor:pointer;font-size:12.5px;font-weight:600;">${t('timetravel_play')}</button>
      </div>
      <div>
        ...
        ${EVENTS.map(ev => `
          <button data-tt-event="${ev.id}" data-tt-date="${ev.date}"
            ...
            <div style="flex:1;min-width:0;">
              ...
              <div style="color:rgba(228,234,248,0.5);font-size:10.5px;">${new Date(ev.date).toLocaleDateString()}</div>
            </div>
  ...
  _toggleAnim() {
    if (this.animInterval) { this._stopAnim(); return; }
