  ...
  _build() {
    ...
    this.panel.innerHTML = `
      <div style="padding:18px 22px;border-bottom:1px solid rgba(255,255,255,0.08);display:flex;justify-content:space-between;align-items:center;">
        <h2 style="margin:0;font-size:1.2rem;font-weight:800;color:#5bc4cf;">${
          t('credits_title') || 'Credits & About'
        }</h2>
      ...
      <div style="flex:1;overflow-y:auto;padding:24px;">
        <div style="text-align:center;margin-bottom:32px;">
          ...
          <p style="margin:8px 0 0;font-size:13px;color:rgba(228,234,248,0.6);">${escapeHtml(
            getLang() === 'it' ? BRAND.tagline : BRAND.taglineEn
          )}</p>
          <p style="margin:4px 0 0;font-size:11px;color:rgba(228,234,248,0.4);">Version ${escapeHtml(
            BRAND.version
          )} · ${escapeHtml(BRAND.year)}</p>
        </div>

        <section style="margin-bottom:28px;">
          <h3 style="font-size:14px;font-weight:700;color:#5bc4cf;margin:0 0 12px;text-transform:uppercase;letter-spacing:1px;">${escapeHtml(
            t('credits_about') || 'About'
          )}</h3>
          <p style="font-size:13px;line-height:1.7;color:rgba(228,234,248,0.8);margin:0;">
            ${escapeHtml(
              t('credits_about_text') ||
                'Astralis è un visualizzatore 3D interattivo del Sistema Solare che utilizza dati orbitali reali dalla NASA/JPL. Esplora pianeti, lune, asteroidi, comete e stelle vicine con precisione scientifica.'
            )}
          </p>
        </section>
        ...
        <section style="margin-bottom:28px;">
          <h3 style="font-size:14px;font-weight:700;color:#5bc4cf;margin:0 0 12px;text-transform:uppercase;letter-spacing:1px;">${escapeHtml(
            t('credits_features') || 'Features'
          )}</h3>
          <ul style="font-size:13px;line-height:1.8;color:rgba(228,234,248,0.8);margin:0;padding-left:20px;">
            ${(t('credits_feat') || []).map((f) => `<li>${escapeHtml(f)}</li>`).join('')}
          </ul>
        </section>

        <section style="margin-bottom:28px;">
          <h3 style="font-size:14px;font-weight:700;color:#5bc4cf;margin:0 0 12px;text-transform:uppercase;letter-spacing:1px;">${escapeHtml(
            t('credits_tech') || 'Technologies'
          )}</h3>
        ...
        <section style="margin-bottom:28px;">
          ...
          <p style="font-size:13px;line-height:1.7;color:rgba(228,234,248,0.8);margin:0;">
            ${escapeHtml(
              t('credits_team_text') ||
                "Sviluppato con passione per l'astronomia e l'educazione scientifica."
            )}
          </p>
        </section>
        ...
        <section>
          <h3 style="font-size:14px;font-weight:700;color:#5bc4cf;margin:0 0 12px;text-transform:uppercase;letter-spacing:1px;">${escapeHtml(
            t('credits_license') || 'License'
          )}</h3>
          <p style="font-size:13px;line-height:1.7;color:rgba(228,234,248,0.8);margin:0;">
            ${escapeHtml(
              t('credits_license_text') ||
                'Open source under MIT License. Dati NASA di pubblico dominio.'
            )}
          </p>
        </section>
