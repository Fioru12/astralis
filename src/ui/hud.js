...
export class HUD {
  ...
  init() {
    ...
    this.el.innerHTML = `<div><span style="color:#5bc4cf;">Data</span> <span id="hudDate">--</span></div>
      <div><span style="color:#5bc4cf;">Focus</span> <span id="hudBody">${t(
        'hud_free'
      )}</span></div>
  ...
  setDate(d) {
    const e = this.el?.querySelector('#hudDate');
    if (e) e.textContent = d.toLocaleDateString(getLang() === 'it' ? 'it-IT' : 'en-US');
  }
  setBody(n) {
    const e = this.el?.querySelector('#hudBody');
    if (e) e.textContent = n || t('hud_free');
  }
  updateFPS(f) {
